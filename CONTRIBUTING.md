...
Obrigado pelo interesse em contribuir com o `sintetizador-dessem`! Este guia descreve
...
fix(deck): correct reading of PDO file when stage is missing
docs: update CONTRIBUTING.md with pre-commit instructions
chore: bump idessem to 1.1.0
```

## Dependencias do Modulo idessem

O modulo [idessem](https://github.com/rjmalves/idessem) e a dependencia central do
`sintetizador-dessem`. Ele fornece a modelagem de cada arquivo de entrada e saida do
modelo DESSEM atraves do framework [cfinterface](https://github.com/rjmalves/cfi).

No `sintetizador-dessem`, a dependencia do `idessem` e concentrada na classe `Deck`,
